(function ($, window, document, undefined) {
  ...
  SpryValidator.prototype = {
    ...
    submitHandler: function () {
      if (_this.$container.prop("tagName") == "FORM") {
        // Form case
    ...
    selectWrapper: function (input, inputContainerId, type) {
      input.wrap(
        `<span id="${inputContainerId}" class="spryValidate" data-spry=${type}></span>`
      ).after(`
      ...
      _this.validationHandler("Select", inputContainerId, [
        _this.defaults.select,
      ]);
    },
    ...
    passwordWrapper: function (input, inputContainerId, type) {
      ...
      _this.validationHandler("Password", inputContainerId, [
        _this.getHtmlOptions(input, _this.defaults.password),
      ]);
    ...
    confirmWrapper: function (input, inputContainerId, type) {
      ...
      _this.validationHandler("Confirm", inputContainerId, [
        _this.idToConfirm,
        _this.getHtmlOptions(input, _this.defaults.confirm),
      ]);
    },

    textareaWrapper: function (input, inputContainerId, type) {
      input.wrap(
        `<span id="${_this.inputContainerId}" class="spryValidate" data-spry=${type}></span>`
      ).after(`
      ...
      _this.validationHandler("Textarea", inputContainerId, [
        _this.getHtmlOptions(input, _this.defaults.textarea),
      ]);
    },
    ...
    radioWrapper: function (input, inputContainerId, type) {
      ...
      _this.validationHandler("Radio", inputContainerId, [
        _this.defaults.radio,
      ]);
    },
    ...
    checkboxWrapper: function (input, inputContainerId, type) {
      ...
      _this.validationHandler("Checkbox", inputContainerId, [
        _this.defaults.checkbox,
      ]);
    },
    ...
    customWrapper: function (input, inputContainerId, type) {
      input.wrap(
        `<span id=${
          input.data("id") ? input.data("id") : inputContainerId
        } class="spryValidate" data-spry=${type}></span>`
      ).after(`
      ...
      if (_this.defaults[input.data("id")]) {
        _this.validationHandler("TextField", input.data("id"), [
          type,
          _this.getHtmlOptions(input, _this.defaults[input.data("id")]),
        ]);
      } else {
        _this.validationHandler("TextField", inputContainerId, [
          "none",
          _this.defaults.none,
        ]);
    ...
    defaultWrapper: function (input, inputContainerId, type) {
      ...
      if (_this.defaults[type]) {
        _this.validationHandler("TextField", inputContainerId, [
          type,
          _this.getHtmlOptions(input, _this.defaults[type]),
        ]);
      } else {
        _this.validationHandler("TextField", inputContainerId, [
          "none",
        ...
        console.log(
          `This data-spry (${type}) type is not currect in span with id (${inputContainerId}), it wont work correctly`
        );
      }
    },

    validationHandler: function (type, inputContainerId, settings) {
      _this.validations[inputContainerId] = new Spry.Widget[
        `Validation${type}`
      ](inputContainerId, ...settings);
    },

    initSpry: function () {
      this.$container.find("[data-spry]").each(function () {
        _this.inputContainerId =
          "spryContainer-" + Math.random().toString(16).slice(2);
        ...
        if (parentClassName !== "spryValidate" && parentNodeName !== "SPAN") {
          if (_this.drawFunctions[spryType]) {
            _this.drawFunctions[spryType](
              $input,
              _this.inputContainerId,
              spryType
            );
          } else {
            _this.drawFunctions["default"](
              $input,
              _this.inputContainerId,
              spryType
            );
